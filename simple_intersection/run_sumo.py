...
if __name__ == "__main__":
    ...
    for strategy, params in strategies.items():
        ...
        strategy_results[strategy] = {
            ...
            "waiting": sum(waitings) / N_FINAL_RUNS,
            "duration": sum(durations) / N_FINAL_RUNS,
            "loss": sum(losses) / N_FINAL_RUNS,
            "warmup": warmup
        }

    print("\n" + "=" * 80)
    print("               FINAL STRATEGY COMPARISON (CRN)")
    print("=" * 80)
    print(f"{'Strategy':<15} {'Warm-up (s)':<12} {'Vehicles':<10} {'Waiting (s)':<12} {'Duration (s)':<12} {'Loss (s)':<12}")
    print("-" * 80)

    for name, r in strategy_results.items():
        print(f"{name.upper():<15} {r['warmup']:<12} {r['vehicles']:<10.0f} "
              f"{r['waiting']:<12.2f} {r['duration']:<12.2f} {r['loss']:<12.2f}")
